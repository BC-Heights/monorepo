...
export const CachedGetPosts = async (
...
export const GetPosts = async (
  ...
  const { data } = await getClient().query<GetPostsQuery>({
    query: GetPostsDocument,
    variables: { first: first, imgSize: imgSize },
    context: { next: { tags: tags } },
  });
  return data;
};

export const CachedGetPostBySlug = async (
...
export const CachedGetImageUrl = async (
...
export const CachedGetAuthorInfo = async (
  { id, type }: GetAuthorInfoQueryVariables,
  tags = ['posts']
) => {
  const GetAuthorInfo = unstable_cache(
    async (id, type) => {
...
export const CachedGetAuthorPosts = async (
  { id, type, imgSize }: GetAuthorPostsQueryVariables,
  tags = ['posts']
) => {
  const GetAuthorPosts = unstable_cache(
    async (id, type, imgSize) => {
